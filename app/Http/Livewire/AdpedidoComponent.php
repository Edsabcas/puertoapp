...
class AdpedidoComponent extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.adpedido-component',compact('categorias','mesas'));
    }
    ...
    public function cargarlista($idc){
        ...
        public function seleccionarplatillo($idcat2,$id,$titulo,$descripcion,$costo,$boquita,$sin_guarnicion,$cambio) {
            ...
            $this->cambioaplcat=$cambio;
            $sql = "SELECT * FROM tb_categoria_platillo where ESTADO=1";
            $categorias= DB::select($sql);
            if($this->cambioaplcat==0){
        ...
        public function crearpedido(){
            ...
            $minuto=date("i");
            $segun=date("s");
            $nopedidoorden='pe'.$anio.$mes.$dia.$hora.$minuto.$segun."-".$this->noMesa;
            if($this->noMesa!=null){
                $sql = "SELECT * FROM tb_tem_pedido where no_tem_pedido=? and id_usuario=? and id_mesa=?";
                $orden= DB::select($sql,array($this->notem,auth()->user()->id,$this->idMesa));
            ...
            if($orden!=null){
                ...
                if($this->noMesa!=null){
                    DB::beginTransaction();
                    if(DB::table('tb_pedidos')->insert(
                        ['NO_PEDIDO' => $nopedidoorden,
                         'ID_MESA' =>$this->idMesa,
                ...
                elseif($this->cat2!=null){
                    ...
                    if(DB::table('tb_pedidos')->insert(
                        ['NO_PEDIDO' => $nopedidoorden,
                         ...
                         'ESTADO_PEDIDO' => 0,
                         'FECHA_CREACION_PEDIDO' =>date('Y-m-d H:i:s'),
                         'tipo_pedido' =>$this->cat2,
                         'nombre_cliente' =>$this->nombre_orden2,
                         'telefono' =>$this->telefono,
                         'direccion_pedido' =>$this->direccion,
                      ]))
                      {
                        DB::commit();
                      }
                ...
                elseif($this->cat3!=null){
                    ...
                    if(DB::table('tb_pedidos')->insert(
                        ['NO_PEDIDO' => $nopedidoorden,
                         ...
                         'ESTADO_PEDIDO' => 0,
                         'FECHA_CREACION_PEDIDO' =>date('Y-m-d H:i:s'),
                         'tipo_pedido' =>$this->cat3,
                         'nombre_cliente' =>$this->nombre_orden3,
                ...
                $idped="";
                $sql = "SELECT ID_PEDIDO FROM tb_pedidos where NO_PEDIDO=?";
                $pedido= DB::select($sql,array($nopedidoorden));
